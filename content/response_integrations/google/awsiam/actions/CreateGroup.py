...
from ..core.consts import INTEGRATION_NAME, CREATE_GROUP
...
def main():
    ...
    group_usernames = extract_action_param(
        siemplify,
    ...
    try:
        ...
        group_usernames = utils.load_csv_to_list(
            group_usernames,
            "Group Name"
        )
        ...
        for group_name in group_usernames:
            try:
                siemplify.LOGGER.info(
                    f"Checking if the group name: {group_name} is a valid group name"
                )
                ...
                siemplify.LOGGER.info(
                    f"Successfully created new group with name: {group_name}"
                )
                ...
                siemplify.LOGGER.error(
                    f"Could not add the following group to IAM {group_name} Names must "
                ...
                siemplify.LOGGER.info(
                    f"Could not add the following group to IAM: {group_name}. "
                    "Group usernames must contain only alphanumeric characters "
                ...
                siemplify.LOGGER.info(
                    f"Could not add the following group to IAM: {group_name}. "
                    "Reach to group limitation in your aws account."
                )
                siemplify.LOGGER.exception(error)

        if already_exists_group_usernames:
            output_message += (
                "Could not add the following groups to IAM: "
                f"{', '.join(already_exists_group_usernames)}. "
        ...
        if invalid_group_usernames:
            output_message += (
                "Could not add the following groups to IAM: "
                f"{', '.join(invalid_group_usernames)}. Group names must contain only "
        ...
        if after_reached_max:
            output_message += (
                ...
                "Reach to Groups limitation in your aws account. \n"
            )
        ...
        if created_groups:
            ...
            output_message = (
                "Successfully added the following groups to IAM: "
        ...
        siemplify.LOGGER.error(
            f"Error executing action {CREATE_GROUP}. Reason: {error}"
        )
        siemplify.LOGGER.exception(error)
