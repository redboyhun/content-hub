...
from ..core.consts import INTEGRATION_NAME, ADD_USER_TO_GROUP_SCRIPT_NAME
...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_NAME} - {ADD_USER_TO_GROUP_SCRIPT_NAME}"
    ...
    try:
        ...
        for user_name in user_names:
            try:
                ...
                manager.add_user_to_group(group_name=group_name, user_name=user_name)
                ...
                siemplify.LOGGER.error(
                    f"Could not add {user_name} to {group_name} because it attempted "
                    "to create resources beyond the current AWS account limits."
                )
                siemplify.LOGGER.exception(error)
        ...
        if successful_users:
            if len(successful_users) == 1:
                siemplify.LOGGER.info(
                    f"Successfully added user {', '.join(successful_users)} to "
                    f"IAM group: {group_name}"
                )
                output_message += (
                    f"Successfully added user {', '.join(successful_users)} to IAM "
                    f"group: {group_name}\n"
                )
            else:
                siemplify.LOGGER.info(
                    f"Successfully added users {', '.join(successful_users)} to IAM "
                    f"group: {group_name}"
                )
                output_message += (
                    f"Successfully added users {', '.join(successful_users)} to IAM "
        ...
        if exceeding_limit_usernames:
            output_message += (
                f"Could not add {', '.join(exceeding_limit_usernames)} to {group_name} "
                "because it attempted to create resources beyond the "
        ...
        if failed_users:
            output_message += (
                f"Could not add {', '.join(failed_users)} to {group_name}."
            )

    except Exception as error:
        siemplify.LOGGER.error(
            f"Error executing action '{ADD_USER_TO_GROUP_SCRIPT_NAME}'. Reason: {error}"
        )
        siemplify.LOGGER.exception(error)
