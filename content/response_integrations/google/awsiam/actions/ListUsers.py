...
def main():
    ...
    max_users_to_return = extract_action_param(
        siemplify,
    ...
    try:
        ...
        if not DEFAULT_MIN_RESULTS <= max_users_to_return <= DEFAULT_MAX_RESULTS:
            raise AWSIAMValidationException(
                f'Valid Range of "Max Users to Return" is: [{DEFAULT_MIN_RESULTS}-'
        ...
        output_message += (
            "Successfully listed available users in AWS IAM."
            if json_results
            else "No users found in" " AWS IAM \n"
        )
