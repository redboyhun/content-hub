...
def main():
    ...
    if max_tags_to_return <= 0:
        siemplify.LOGGER.info(
            f"Max Tags to Return must be positive. "
            f"Using default value instead ({DEFAULT_LIMIT_TAGS})."
        )
    ...
    try:
        ...
        if tags:
            siemplify.LOGGER.info(f"Found {len(tags)} tags.")
            ...
            output_message += (
                "Successfully listed available tags in McAfee Mvision ePO V2"
            )

        else:
            siemplify.LOGGER.info("No tags were found.")
            output_message += (
                "Action wasn’t able to list tags available in McAfee Mvision ePO V2"
            )
            result_value = False
