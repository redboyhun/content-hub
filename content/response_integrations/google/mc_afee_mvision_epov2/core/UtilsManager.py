...
def read_ids(siemplify, ids_file_name="ids.json", max_hours_backwards=24):
    ...
    :param max_hours_backwards: {int} Max amount of hours to keep ids in
                                the file (to prevent it from getting too big)
    :param ids_file_name: {str} The name of the ids file
    :return:{dict} A dict describing the already seen ids {id: the
            unixtime when it was first seen}
    """
        ...
        with open(ids_file_path, "r") as f:
            ...
            for alert_id, timestamp in existing_ids.items():
                if (
                    timestamp
                    > arrow.utcnow().shift(
                        hours=-max_hours_backwards).timestamp() * 1000
                ):
    ...
    try:
        ...
        with open(ids_file_path, "w") as f:
            try:
                ...
                f.seek(0)
                # Empty the content of the file (the partially written content that
